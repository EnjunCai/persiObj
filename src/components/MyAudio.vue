<template>
  <div
    v-if="useMusicStoreData.currentMusic?.play"
    class="audioWrapper"
    :style="{
  ...
  >
    ...
    <div>{{ currentTimeFormatted }} / {{ totalTimeFormatted }}</div>
    <!-- 音乐进度条 -->
    <div
      class="musicProgress"
      ref="containerRect"
      @mousedown="handleProgressBarDown($event)"
...
const handleMouseMove = (e: MouseEvent) => {
  ...
  const newProgressPercentage = Math.min(
    Math.max((newProgressX / containerRect1.width) * 100, 0),
    100
  );
